...
import logging
logging.basicConfig()
evohome_log = logging.getLogger("evohomeBridge")
...
try:
	...
	if lastResponse.find("TooManyRequests") == -1:
	...
	if loc == None:
		...
		for zone in tcs._zones:
			# specific case (from jaktens-2018-11) [or zone.temperatureStatus['isAvailable'] == True]
			if zone.name.strip():
				nb += 1
				if nb > 1:
					jZones = jZones + ','
	...
	if lastResponse != None:
		evohome_log.info("lastResponse : " + lastResponse)
