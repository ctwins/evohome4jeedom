...
class ControlSystem(EvohomeBase):
	...
	def _set_status(self, mode, until=None):
		...
		r = requests.put(self.client.baseurl+'/temperatureControlSystem/%s/mode' % self.systemId, data=json.dumps(data), headers=headers, timeout=self.client.request_timeout)
