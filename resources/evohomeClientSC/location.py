...
class Location(EvohomeBase):
	...
	def status(self):
		r = requests.get(self.client.baseurl+'/location/%s/status?includeTemperatureControlSystems=True' % self.locationId, headers=self.client.headers(), timeout=self.client.request_timeout)
